...
const TodayLoginScreen: React.FC = () => {
  const { userInfo } = useAuth();
  const [todayLogins, setTodayLogins] = useState<User[]>([]);
  const [todaySignups, setTodaySignups] = useState<User[]>([]);
  const [hourlyData, setHourlyData] = useState<HourlyData[]>([]);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [totalTodayLogins, setTotalTodayLogins] = useState(0);
  const [totalTodaySignups, setTotalTodaySignups] = useState(0);
  ...
  const fetchTodayLogins = async () => {
    try {
      ...
      const todayLoginUsers: User[] = [];
      const todaySignupUsers: User[] = [];
      const hourlyLoginMap = new Map<number, number>();
      const hourlySignupMap = new Map<number, number>();

      for (let i = 0; i < 24; i++) {
        hourlyLoginMap.set(i, 0);
        hourlySignupMap.set(i, 0);
      }
      ...
      snapshot.forEach((doc) => {
        ...
        if (lastLoginAt) {
          ...
          try {
            ...
            if (loginDate >= startOfDay && loginDate <= endOfDay) {
              ...
              todayLoginUsers.push(user);
             

              const hour = loginDate.getHours();
              hourlyLoginMap.set(hour, (hourlyLoginMap.get(hour) || 0) + 1);
            }
          } catch (error) {
            console.error(
        ...
        if (createdAt) {
          ...
          try {
            ...
            if (signupDate >= startOfDay && signupDate <= endOfDay) {
              const user: User = {
                uid: doc.id,
                email: userData.email || "",
                displayName: userData.displayName || "",
              ...
              todaySignupUsers.push(user);
            
              // Count hourly signups
              const hour = signupDate.getHours();
              hourlySignupMap.set(hour, (hourlySignupMap.get(hour) || 0) + 1);
            }
          } catch (error) {
            console.error(
      ...
      for (let i = 0; i < 24; i++) {
        chartData.push({
          ...
          loginCount: hourlyLoginMap.get(i) || 0,
          signupCount: hourlySignupMap.get(i) || 0,
        });
      }

     
      setTodayLogins(todayLoginUsers);
      setTodaySignups(todaySignupUsers);
      setHourlyData(chartData);
      setTotalTodayLogins(todayLoginUsers.length);
      setTotalTodaySignups(todaySignupUsers.length);
    } catch (error) {
      ...
      return (
        <View
          style={[
            ...
            {
              // backgroundColor: "rgba(121, 170, 0, 0.03)",
              // elevation: 8,
              ...
              // shadowRadius: 12,
              // borderWidth: 1,
              // borderColor: "rgba(121, 170, 0, 0.08)",
              // height: height,
            },
          ]}
        >
      ...
      >
        ...
        <View style={tw`px-4  `}>
          ...
          <View style={tw`mb-4  flex items-center justify-center`}>
            <CustomPieChart
              totalLogins={totalTodayLogins}
              totalSignups={totalTodaySignups}
        ...
        <View style={tw`px-4 mb-6`}>
          ...
          >
            Today's Login Users ({todayLogins.length})
          </Text>
          ...
          {todayLogins.length === 0 ? (
            ...
            <View style={tw`mb-4`}>
              {todayLogins
                ...
                .slice(0, showMoreLogins ? todayLogins.length : 5)
                .map((user) => (
                  <View
                    key={`login-${user.uid}`}
                    style={[
                ...
                >
                  <View style={tw`flex-row items-center`}>
                    ...
                    >
                      {showMoreLogins
                        ? "Show Less"
                        : `Show ${todayLogins.length - 5} More`}
                    </Text>
                  </View>
                </TouchableOpacity>
              )}
        ...
        <View style={tw`px-4 mb-6`}>
          ...
          >
            Today's New Signups ({todaySignups.length})
          </Text>
          ...
          {todaySignups.length === 0 ? (
            <View style={tw`items-center py-6 bg-blue-50 rounded-xl`}>
              ...
              >
                No new users signed up today yet
              </Text>
            </View>
          ) : (
            <View style={tw`mb-4`}>
              {todaySignups
                ...
                .slice(0, showMoreSignups ? todaySignups.length : 5)
                .map((user) => (
                  <View
                    key={`signup-${user.uid}`}
                    style={[
                ...
                >
                  <View style={tw`flex-row items-center`}>
                    ...
                    >
                      {showMoreSignups
                        ? "Show Less"
                        : `Show ${todaySignups.length - 5} More`}
                    </Text>
                  </View>
                </TouchableOpacity>
              )}
...
export default TodayLoginScreen;
